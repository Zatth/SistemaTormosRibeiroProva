...
public class VendasDAO extends DAO_Abstract {
    ...
    public Object list(int id) { //aqui o parâmetro
       session.beginTransaction(); // comecei uma transaçao
        Criteria criteria = session.createCriteria(VendasZht.class); // Criteria é uma classe do hibernate para fazer consultas, to criando ela
        // basicamente um Select * from Usuarios
        criteria.add(Restrictions.eq("id_vendas_zht", id)); // o ID vem do parâmetro, essa linha é tipo um where
    ...
    public ArrayList listAll() {
        session.beginTransaction();
        Criteria criteria = session.createCriteria(VendasZht.class);
